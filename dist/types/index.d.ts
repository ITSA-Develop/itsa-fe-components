...
export type TModalSize = ESize.large | ESize.medium | ESize.small;
...
export type TTableData = {
    [key: string]: unknown;
    TABLE__ALERT?: boolean;
}[];
export type TSortOrder = ESortOrder.asc | ESortOrder.desc;
...
export type PermissionAction = keyof IActions;
export type TTab = {
    icon: string;
    label: string;
    content: ReactNode;
...
export type RHFInputProps<TFieldValues extends FieldValues> = Omit<InputProps, 'value' | 'onChange'> & BaseFieldProps & (RHFModeProps<TFieldValues> | ControlledModeProps<string>);
