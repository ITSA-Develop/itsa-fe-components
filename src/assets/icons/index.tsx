import ActiveNotificationIcon from './icon-active-notifications.svg?react';
import AddListIcon from './icon-add-list.svg?react';
import AddProductIcon from './icon-add-product.svg?react';
import AddToCartIcon from './icon-add-to-cart.svg?react';
import AiIcon from './icon-ai.svg?react';
import AlertIcon from './icon-alert.svg?react';
import AppleIcon from './icon-apple.svg?react';
import ArrowLeftIcon from './icon-arrow-left.svg?react';
import ArrowRightIcon from './icon-arrow-right.svg?react';
import AttachmentIcon from './icon-attachment.svg?react';
import BillIcon from './icon-bill.svg?react';
import CalcIcon from './icon-calc.svg?react';
import CalendarIcon from './icon-calendar.svg?react';
import CameraIcon from './icon-camera.svg?react';
import IconCamion from './icon-camion.svg?react';
import CardIcon from './icon-card.svg?react';
import CartIcon from './icon-cart.svg?react';
import CashbackIcon from './icon-cashback.svg?react';
import CheckIcon from './icon-check.svg?react';
import CheckCircleIcon from './icon-check-circle.svg?react';
import ChevronDobleDerechaIcon from './icon-chevron-doble-derecha.svg?react';
import ChevronDownIcon from './icon-chevron-down.svg?react';
import ChevronLeftIcon from './icon-chevron-left.svg?react';
import ChevronRightIcon from './icon-chevron-right.svg?react';
import ChevronUpIcon from './icon-chevron-up.svg?react';
import ColaboradoreIcon from './icon-colaboradore.svg?react';
import CouponIcon from './icon-coupon.svg?react';
import CurveIcon from './icon-curve.svg?react';
import EditIcon from './icon-edit.svg?react';
import EmailIcon from './icon-email.svg?react';
import EyeIcon from './icon-eye.svg?react';
import EyeOffIcon from './icon-eye-off.svg?react';
import FeriadosIcon from './icon-feriados.svg?react';
import FilterIcon from './icon-filter.svg?react';
import FiltersIcon from './icon-filters.svg?react';
import GolfIcon from './icon-golf.svg?react';
import GoogleIcon from './icon-google.svg?react';
import GoogleMapsIcon from './icon-google-maps.svg?react';
import HeartIcon from './icon-heart.svg?react';
import HistoryIcon from './icon-history.svg?react';
import HomeIcon from './icon-home.svg?react';
import IdIcon from './icon-id.svg?react';
import InfoIcon from './icon-info.svg?react';
import InvoiceIcon from './icon-invoice.svg?react';
import KeyIcon from './icon-key.svg?react';
import LimpiarNotificacionesIcon from './icon-limpiar-notificaciones.svg?react';
import ListIcon from './icon-list.svg?react';
import LoaderIcon from './icon-loader.svg?react';
import LockIcon from './icon-lock.svg?react';
import LogoutIcon from './icon-logout.svg?react';
import MenuIcon from './icon-menu.svg?react';
import MenuActiveIcon from './icon-menu-active.svg?react';
import MenuDotsIcon from './icon-menu-dots.svg?react';
import MetaIcon from './icon-meta.svg?react';
import MicIcon from './icon-mic.svg?react';
import Mic1Icon from './icon-mic-1.svg?react';
import MinusProductIcon from './icon-minus-product.svg?react';
import MotitoIcon from './icon-motito.svg?react';
import NegociosIcon from './icon-negocios.svg?react';
import NotificationIcon from './icon-notifications.svg?react';
import OfacIcon from './icon-ofac.svg?react';
import OnuIcon from './icon-onu.svg?react';
import OrderIcon from './icon-order.svg?react';
import OrdernarIcon from './icon-ordernar.svg?react';
import PackageIcon from './icon-package.svg?react';
import PepsIcon from './icon-peps.svg?react';
import PhoneIcon from './icon-phone.svg?react';
import PhtoAlbumIcon from './icon-phto-album.svg?react';
import PigIcon from './icon-pig.svg?react';
import PinIcon from './icon-pin.svg?react';
import Pin2Icon from './icon-pin-2.svg?react';
import PlusIcon from './icon-plus.svg?react';
import PreparacionIcon from './icon-preparacion.svg?react';
import PresupuestoIcon from './icon-presupuesto.svg?react';
import PromoIcon from './icon-promo.svg?react';
import Promo1Icon from './icon-promo-1.svg?react';
import ProvidenciasIcon from './icon-providencias.svg?react';
import PuntosIcon from './icon-puntos.svg?react';
import ReclamosIcon from './icon-reclamos.svg?react';
import RegalosIcon from './icon-regalos.svg?react';
import RemoveItemIcon from './icon-remove-item.svg?react';
import ScanQrIcon from './icon-scan-qr.svg?react';
import SearchIcon from './icon-search.svg?react';
import SecurityIcon from './icon-security.svg?react';
import SendIcon from './icon-send.svg?react';
import SettingsIcon from './icon-settings.svg?react';
import ShareIcon from './icon-share.svg?react';
import ShieldIcon from './icon-shield.svg?react';
import ShopIcon from './icon-shop.svg?react';
import ShopActiveIcon from './icon-shop-active.svg?react';
import ShoppingCartIcon from './icon-shopping-cart.svg?react';
import ShoppingCart2Icon from './icon-shopping-cart-2.svg?react';
import ShoppingCartActiveIcon from './icon-shopping-cart-active.svg?react';
import SmartphoneIcon from './icon-smartphone.svg?react';
import SquareCheckIcon from './icon-square-check.svg?react';
import SquareRoundedPlusIcon from './icon-square-rounded-plus.svg?react';
import StarIcon from './icon-star.svg?react';
import StoreIcon from './icon-store.svg?react';
import TransportistasIcon from './icon-transportistas.svg?react';
import TrashIcon from './icon-trash.svg?react';
import TrophyIcon from './icon-trophy.svg?react';
import UploadIcon from './icon-upload.svg?react';
import UserIcon from './icon-user.svg?react';
import User2Icon from './icon-user2.svg?react';
import UserActiveIcon from './icon-user-active.svg?react';
import UserRoundedSquareIcon from './icon-user-rounded-square.svg?react';
import VehicleIcon from './icon-vehicle.svg?react';
import WalletIcon from './icon-wallet.svg?react';
import WalletIcon2 from './icon-wallet-2.svg?react';
import WhatsappIcon from './icon-whatsapp.svg?react';
import WithoutInformationIcon from './icon-without-information.svg?react';
import WorkshopIcon from './icon-workshop.svg?react';
import XIcon from './icon-x.svg?react';
import XCircleIcon from './icon-x-circle.svg?react';
import HouseholdIcon from './icon-household.svg?react';


export {
  ActiveNotificationIcon,
  AddListIcon,
  AddProductIcon,
  AddToCartIcon,
  AiIcon,
  AlertIcon,
  AppleIcon,
  ArrowLeftIcon,
  ArrowRightIcon,
  AttachmentIcon,
  BillIcon,
  CalcIcon,
  CalendarIcon,
  CameraIcon,
  IconCamion,
  CardIcon,
  CartIcon,
  CashbackIcon,
  CheckIcon,
  CheckCircleIcon,
  ChevronDobleDerechaIcon,
  ChevronDownIcon,
  ChevronLeftIcon,
  ChevronRightIcon,
  ChevronUpIcon,
  ColaboradoreIcon,
  CouponIcon,
  CurveIcon,
  EditIcon,
  EmailIcon,
  EyeIcon,
  EyeOffIcon,
  FeriadosIcon,
  FilterIcon,
  FiltersIcon,
  GolfIcon,
  GoogleIcon,
  GoogleMapsIcon,
  HeartIcon,
  HistoryIcon,
  HomeIcon,
  IdIcon,
  InfoIcon,
  InvoiceIcon,
  KeyIcon,
  LimpiarNotificacionesIcon,
  ListIcon,
  LoaderIcon,
  LockIcon,
  LogoutIcon,
  MenuIcon,
  MenuActiveIcon,
  MenuDotsIcon,
  MetaIcon,
  MicIcon,
  Mic1Icon,
  MinusProductIcon,
  MotitoIcon,
  NegociosIcon,
  NotificationIcon,
  OfacIcon,
  OnuIcon,
  OrderIcon,
  OrdernarIcon,
  PackageIcon,
  PepsIcon,
  PhoneIcon,
  PhtoAlbumIcon,
  PigIcon,
  PinIcon,
  Pin2Icon,
  PlusIcon,
  PreparacionIcon,
  PresupuestoIcon,
  PromoIcon,
  Promo1Icon,
  ProvidenciasIcon,
  PuntosIcon,
  ReclamosIcon,
  RegalosIcon,
  RemoveItemIcon,
  ScanQrIcon,
  SearchIcon,
  SendIcon,
  SettingsIcon,
  SecurityIcon,
  ShareIcon,
  ShieldIcon,
  ShopIcon,
  ShopActiveIcon,
  ShoppingCartIcon,
  ShoppingCart2Icon,
  ShoppingCartActiveIcon,
  SmartphoneIcon,
  SquareCheckIcon,
  SquareRoundedPlusIcon,
  StarIcon,
  StoreIcon,
  TransportistasIcon,
  TrashIcon,
  TrophyIcon,
  UploadIcon,
  UserIcon,
  User2Icon,
  UserActiveIcon,
  UserRoundedSquareIcon,
  VehicleIcon,
  WalletIcon,
  WalletIcon2,
  WhatsappIcon,
  WithoutInformationIcon,
  WorkshopIcon,
  XIcon,
  XCircleIcon,
  HouseholdIcon,
};
