...
Proporciona un Manejador de Cambios: Expone una única función (onChangePaginationHook) que se conecta al evento onChange de la tabla. Esta función se encarga de recibir las nuevas configuraciones de paginación, filtros y ordenamiento de la tabla y actualizar el estado interno del hook.
...
- \ onChangePaginationHook: La función controladora que actualiza todos los estados anteriores cuando el usuario interactúa con la tabla.
...
const TableWithPaginationState = () => {
	const { pagination, filters, sorter, onChangePaginationHook } = useTable(DEFAULT_PAGINATION_CONFIG);
	...
	return (
		<div className="flex flex-col gap-2">
			...
			<Table
				...
				onChange={onChangePaginationHook}
				rowKey={'name'}
			/>
		</div>
	);
